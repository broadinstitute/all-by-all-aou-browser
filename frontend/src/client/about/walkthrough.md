...
* **Side by side:** On sufficiently wide screens, choose **Side by side** to keep broader phenotype- or genome-wide **Results** visible on the left while inspecting detailed data for a selected item on the right. This is useful when moving through results without repeatedly navigating away from the larger context. The layout controls show Results only, Split, or Details only, and the central dotted divider resizes the panes in Split view. Pane width can affect which controls or table columns are visible.
