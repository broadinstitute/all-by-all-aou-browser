...
function generateTicks(): number[] {
  ...
  // Linear region: 0 to logThreshold (typically 0-10)
  // Use integer steps
  for (let i = 0; i <= LOG_THRESHOLD; i += 2) {
    ticks.push(i);
  }

  // Log region: above logThreshold
  // Use powers of 10 and intermediate values
  const logStart = Math.ceil(Math.log10(LOG_THRESHOLD));
  const logEnd = Math.floor(Math.log10(MAX_NEG_LOG_P));

  for (let exp = logStart; exp <= logEnd; exp++) {
    const val = Math.pow(10, exp);
    if (val > LOG_THRESHOLD && val <= MAX_NEG_LOG_P) {
      ticks.push(val);
    }
    // Add 3x intermediate for better coverage
    const intermediate = 3 * Math.pow(10, exp);
    if (intermediate > LOG_THRESHOLD && intermediate <= MAX_NEG_LOG_P) {
 ...
 * This mirrors the YScale in layout.ts.
 */
function valueToY(val: number, height: number): number {
  const linearHeight = height * LINEAR_FRACTION;
  const logHeight = height * (1 - LINEAR_FRACTION);

  if (val <= LOG_THRESHOLD) {
    // Linear portion: maps [0, logThreshold] -> [height, height - linearHeight]
    const normalized = val / LOG_THRESHOLD;
    return height - normalized * linearHeight;
  } else {
    // Log portion: maps [logThreshold, maxNegLogP] -> [height - linearHeight, 0]
    const logVal = Math.log(val / LOG_THRESHOLD);
 ...
 */
export const YAxis: React.FC<YAxisProps> = ({ height, width = 50 }) => {
