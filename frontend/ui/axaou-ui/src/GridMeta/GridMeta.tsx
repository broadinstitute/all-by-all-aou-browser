...
const baseRowStyle = css`
  ...
  align-items: stretch;
  box-sizing: border-box;
  border-top: 1px solid var(--theme-border, #424242);
`;
...
type DataRowProps = {
  data: {
    ...
    metadataKey: string;
    metadataTypeKey: string;
    metadataRender: Record<string, (...args: any[]) => any>;
...
const DataRow = ({
  ...
  const metadataType = rowData[metadataTypeKey]

  let metadataRenderFunction = metadataRender['default']

  if (metadataType in metadataRender) {
    metadataRenderFunction = metadataRender[metadataType]
  }

  const metadataEl = metadataRenderFunction(rowData[metadataKey], rowIndex)

  return (
    <div
      aria-rowindex={rowIndex + 1}
      className={`grid-row ${dataRowIndex % 2 === 0 ? "grid-row-stripe" : ""} ${shouldHighlightRow(rowData) ? "grid-row-highlight" : ""
        }`}
      onMouseEnter={onMouseEnter}
      role="row"
      style={style}
    >
      <div className="grid-row-cols">
      ...
      {rowData[metadataKey] && <div className="metadata-cell">{metadataEl}</div>}
    </div>
  );
};
type OwnGridProps = {
...
type GridProps = OwnGridProps & typeof GridMeta.defaultProps;
export class GridMeta extends Component<GridProps> {
  static defaultProps = {
    cellData: {},
    numRowsRendered: 20,
    onHoverRow: () => { },
