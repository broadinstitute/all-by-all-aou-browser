...
- This returns:
  ...
  - **PheWAS associations**: up to 20 API-ordered phenotype rows with phenotype name, ancestry, sequencing type, p-value, beta, standard error, and allele frequency
...
- Report beta with its standard error when available.
- The sign follows the upstream model's allele, outcome coding, and transformation. Do not call a positive beta "risk increasing" or a negative beta "protective" without those definitions.
- For a documented logistic model, beta is on the log-odds scale and `exp(beta)` is the odds ratio. For a documented linear model, units depend on outcome transformation.
...
1. Note the `ancestry` field for each significant association:
...
2. **Significant Associations Table**:

| Phenotype | P-value | Beta | Ancestry | Seq Type |
|-----------|---------|------|----------|----------|
| ... | ... | ... | ... | ... |

3. **Clinical Interpretation**:
   - Group significant phenotypes by organ system or disease category
   - Note direction of effect (risk-increasing vs protective)
   - Highlight any unexpected or novel associations
   - Identify if the variant's known biology (gene function) is consistent with the phenotypic associations
4. **Caveats**:
